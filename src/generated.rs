...
pub trait SecureGreetingProvider: fmt::Debug + 'static {
    fn greet(&self, context: InvocationContext, request: GreetRequest) -> LocalBoxFuture<'static, Result<GreetResponse, SecureGreetingInvocationError>>;
}

#[derive(Debug)]
pub struct SecureGreetingEndpoint<P> { provider: Rc<P> }
            ...
            GREET_OPERATION => {
                ...
                Box::pin(async move {
                    match provider.greet(context, *request).await {
                        ...
                        Err(SecureGreetingInvocationError::Domain(error)) => Ok(Err(Box::new(error) as Box<dyn std::any::Any>)),
