...
export const CAPABILITY_ID = "example.agent@1";
...
export type RunError = "invalid_prompt" | "memory_rejected" | "model_rejected" | "tool_rejected" | UnknownDomainError;
...
export function encodeRunRequest(value: RunRequest): string { validatePortableJson(value); const wire = JSON.stringify(value); if (wire === undefined) throw new Error("request cannot be encoded"); return wire; }
export function decodeRunRequest(wire: string): RunRequest { const value: unknown = JSON.parse(wire); validatePortableJson(value); return value as RunRequest; }
export function encodeRunResponse(value: RunResponse): string { validatePortableJson(value); const wire = JSON.stringify(value); if (wire === undefined) throw new Error("response cannot be encoded"); return wire; }
export function decodeRunResponse(wire: string): RunResponse { const value: unknown = JSON.parse(wire); validatePortableJson(value); return value as RunResponse; }
export function encodeRunError(value: RunError): string { validatePortableJson(value); const wire = JSON.stringify(value); if (wire === undefined) throw new Error("Domain Error cannot be encoded"); return wire; }
export function decodeRunError(wire: string): RunError {
  const value: unknown = JSON.parse(wire);
  validatePortableJson(value);
  if (typeof value === "string") {
    if (value === "invalid_prompt") return value as RunError;
    if (value === "memory_rejected") return value as RunError;
    if (value === "model_rejected") return value as RunError;
    if (value === "tool_rejected") return value as RunError;
    return { code: value } as RunError;
  }
  if (isRecord(value) && typeof value.code === "string") return value as RunError;
