...
export const CAPABILITY_ID = "example.agent.progress@1";
...
export type UpdateError = "subscriber_failed" | UnknownDomainError;
export type UpdateResult = ReadonlyArray<EventPublishResult>;
...
export function encodeUpdateRequest(value: UpdateRequest): string { validatePortableJson(value); const wire = JSON.stringify(value); if (wire === undefined) throw new Error("request cannot be encoded"); return wire; }
export function decodeUpdateRequest(wire: string): UpdateRequest { const value: unknown = JSON.parse(wire); validatePortableJson(value); return value as UpdateRequest; }
export function encodeUpdateResponse(value: null): string { validatePortableJson(value); const wire = JSON.stringify(value); if (wire === undefined) throw new Error("response cannot be encoded"); return wire; }
export function decodeUpdateResponse(wire: string): null { const value: unknown = JSON.parse(wire); validatePortableJson(value); return value as null; }
export function encodeUpdateError(value: UpdateError): string { validatePortableJson(value); const wire = JSON.stringify(value); if (wire === undefined) throw new Error("Domain Error cannot be encoded"); return wire; }
export function decodeUpdateError(wire: string): UpdateError {
  const value: unknown = JSON.parse(wire);
  validatePortableJson(value);
  if (typeof value === "string") {
    if (value === "subscriber_failed") return value as UpdateError;
    return { code: value } as UpdateError;
  }
  if (isRecord(value) && typeof value.code === "string") return value as UpdateError;
  throw new Error("Domain Error must be a string or object");
}

export function encodeUpdateEvent(value: UpdateRequest): string { validatePortableJson(value); const wire = JSON.stringify(value); if (wire === undefined) throw new Error("event cannot be encoded"); return wire; }
export function decodeUpdateEvent(wire: string): UpdateRequest { const value: unknown = JSON.parse(wire); validatePortableJson(value); return value as UpdateRequest; }


export interface ProgressClient {
  update(event: UpdateRequest, context?: InvocationContext): Promise<UpdateResult>;
}

export interface ProgressProvider {
  update(context: InvocationContext, event: UpdateRequest): void;
}
